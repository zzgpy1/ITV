...
class SourceDiscoverer:
    def __init__(self, pool_db_path: Path = None):
        ...
        self._load_pool()
        # 移除警告，改为调试信息
        logger.debug("SourceDiscoverer 以存根模式运行，实际功能未完全启用")
            ...
            try:
                with open(self.pool_db_path, 'r', encoding='utf-8') as f:
                    ...
                    for key, value in data.items():
                        value["discovered_at"] = datetime.fromisoformat(value.get("discovered_at", datetime.now().isoformat()))
    ...
    async def discover(self, db=None, filter_domestic: bool = True, force_refresh: bool = False) -> Dict[str, List[RawSource]]:
        ...
        raw_contents = await fetch_all_sources_incremental(config.iptv_sources, db, force_refresh)
    ...
    def update_source_status(self, source_key: str, status: str, latency: int = 0, success: bool = True):
