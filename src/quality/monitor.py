...
from src.speed_tester import channel_key
...
class QualityMonitor:
    ...
    async def check_all_active_sources(self):
        ...
        for name, info in active.items():
            ...
            history = await self.history_repo.get_history(key, days=7)
            ...
            success_latencies = [h["latency"] for h in history if h["success"] and h["latency"] > 0]
            avg_latency = sum(success_latencies) // len(success_latencies) if success_latencies else 9999
