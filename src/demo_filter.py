...
from src.classifier import classify_and_filter, PROVINCES
...
try:
    ...
    HAS_PYPINYIN = False
    logger.warning("⚠️ pypinyin 未安装，拼音匹配功能将不可用")
...
def match_channel_name(channel_name: str, demo_name: str) -> bool:
    ...
    if m1 and m2 and m1.group(1).lower() == m2.group(1).lower():
        ...
        if num.isdigit() and 1 <= int(num) <= 17:
            area_keywords = {"欧洲": ["欧洲", "europe"], "美洲": ["美洲", "america", "americas"]}
...
def detect_province(channel_name: str) -> str:
    ...
    city_map = {
        ...
        "武汉": "湖北", "成都": "四川", "长沙": "湖南", "郑州": "河南",
        "石家庄": "河北", "合肥": "安徽", "南昌": "江西", "太原": "山西",
        "沈阳": "辽宁", "长春": "吉林", "哈尔滨": "黑龙江", "西安": "陕西",
        "兰州": "甘肃", "昆明": "云南", "贵阳": "贵州", "福州": "福建",
        "济南": "山东", "青岛": "山东", "厦门": "福建", "宁波": "浙江",
        "苏州": "江苏", "无锡": "江苏", "佛山": "广东", "东莞": "广东",
    }
...
def filter_and_order_by_demo(channels: list) -> tuple:
    ...
    logger.info(f"🎯 Demo 筛选：原始 {len(channels)} -> {len(matched)} 个频道")
    return matched, []
