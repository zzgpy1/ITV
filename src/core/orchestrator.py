...
class Orchestrator:
    ...
    async def run(self, skip_discover: bool = False):
        ...
        if settings.ffmpeg_enable:
            ...
            # 获取已验证但未深度验证的源
            verified = await repo_factory.source._fetchall(
                "SELECT source_key, channel_name, url FROM source_pool WHERE status = 'verified'"
            )
            if verified:
                channels = [{"key": r[0], "name": r[1], "url": r[2]} for r in verified]
        ...
        for cand in stable_candidates:
            ...
            if existing and existing["latency"] < cand["latency"]:
                continue
            if await self.stable_manager.promote_candidate(cand["name"], cand["url"],
                                                           cand["latency"], "h264"):
        ...
        fixed_count = sum(1 for s in stable_all.values() if s["is_fixed"])
